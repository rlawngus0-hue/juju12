...
window.onload = () => {
    ...
    startLiveSimulation(); 
    initLadder();
    const hilowDisplay = document.getElementById('hilow-num-display');
    if (hilowDisplay) hilowDisplay.innerText = hilowCurrentNum;
};
...
function drawLadder() { 
    ...
    ctx.beginPath(); ctx.moveTo(100, 50); ctx.lineTo(100, 450); ctx.moveTo(300, 50); ctx.lineTo(300, 450); ctx.stroke(); 
    ladderPaths.connectors.forEach(y => { ctx.beginPath(); ctx.moveTo(100, y); ctx.lineTo(300, y); ctx.stroke(); }); 
}
async function startLadder() {
    ...
    if (res === currentBet) { points += Math.floor(amount * 1.9); alert('당첨!'); } else alert('실패!');
...
// --- Game Logic: Slot Machine ---
function startSlot() {
    if (isGameRunning) return;
    const amount = parseInt(document.getElementById('slot-bet-amount').value);
    if (isNaN(amount) || amount < 100) { alert('베팅 금액을 확인하세요!'); return; }
    if (amount > points) { alert('보유 포인트가 부족합니다!'); return; }
    isGameRunning = true; points -= amount; updatePointsDisplay();
    ...
    const interval = setInterval(() => {
        ...
        if (rollCount > 15) {
            ...
            if (res[0] === res[1] && res[1] === res[2]) { points += amount * 10; alert('JACKPOT!!!'); }
            else if (res[0] === res[1] || res[1] === res[2] || res[0] === res[2]) { points += amount * 2; alert('당첨!'); }
            else alert('꽝!');
            isGameRunning = false; updatePointsDisplay();
        }
    }, 80);
...
function startHiLow() {
    ...
    setTimeout(() => {
        ...
        let won = (currentBet === 'high' && nextNum > hilowCurrentNum) || (currentBet === 'low' && nextNum < hilowCurrentNum);
        if (won) { points += amount * 2; alert('당첨!'); } else alert('패배!');
        hilowCurrentNum = nextNum; isGameRunning = false; updatePointsDisplay();
    }, 800);
}
...
function saveNickname() { const input = document.getElementById('nickname-input').value.trim(); if (input.length < 2) return; nickname = input; localStorage.setItem('jujuNickname', nickname); points = parseInt(localStorage.getItem('jujuPoints_' + nickname)) || 10000; location.reload(); }
...
function startLiveSimulation() { setInterval(() => { rankings.forEach(p => { if (p.name !== nickname) { const change = Math.floor(Math.random() * 20000) * (Math.random() > 0.5 ? 1 : -1); p.points = Math.max(0, p.points + change); } }); updateRanking(); }, 6000); }
